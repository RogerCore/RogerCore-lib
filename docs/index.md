...
TheHolyRoger is a powerful new peer-to-peer platform for the next generation of financial technology. The decentralized nature of the TheHolyRoger network allows for highly resilient ROGER infrastructure, and the developer community needs reliable, open-source tools to implement ROGER apps and services. RogerCore provides a reliable API for JavaScript apps that need to interface with TheHolyRoger.

To get started, just `npm install rogercore` or `bower install rogercore`.
...
* [Peer to Peer Networking](https://github.com/RogerCore/RogerCore-p2p)
...
var imported = rogercore.PrivateKey.fromWIF(exported);
...
var p2shAddress = new rogercore.Address([publicKey1, publicKey2, publicKey3], 2);
```
...
var uri = new rogercore.URI(paymentInfo).toString();
```
