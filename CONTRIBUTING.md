Contributing to RogerCore
=======

We're working hard to make *RogerCore* the most powerful JavaScript library for working with TheHolyRoger. Our goal is to have *RogerCore* be a library that can be used by anyone interested in TheHolyRoger, and to level expertise differences with great design and documentation.
...
These are some global design goals in RogerCore that any change must adhere.
...
When possible, RogerCore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of RogerCore methods. For example, calling `JSON.stringify` on a RogerCore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'rogercore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the RogerCore object.

DO:
```javascript
var rogercore = require('../');
var PublicKey = rogercore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add RogerCore git@github.com:RogerCore/RogerCore.git
git pull --rebase RogerCore master
```
...
```
Finally go to [github.com/RogerCore/RogerCore](https://github.com/RogerCore/RogerCore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of RogerCore.

If you have any questions feel free to post them to
[github.com/RogerCore/RogerCore/issues](https://github.com/RogerCore/RogerCore/issues).
